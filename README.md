...
- **D-Scan 解析** — 粘贴 EVE 扫描结果，自动识别中/英文舰船名，按作战功能分组（后勤/特种/火力/运输）
- **ECM 建议** — 根据扫描到的舰船势力自动推荐对应 ECM 干扰类型（雷达/引力/磁力/光雷达/多谱式）
- **舰船浏览器** — 浏览全部 531 艘舰船的中文数据，支持搜索和分类筛选，点击船名查看详情
- **舰船详情弹窗** — 点击船名查看空船属性（质量/槽位/HP/电容/无人机等）+ 船体加成（中文 traits）
- **ESI 查询** — 支持粘贴角色名列表，自动查询军团/联盟（需 PHP 后端）
...
├── ships-data.js    # 舰船数据（531 条，CCP SDE + ESI）
├── traits-data.js   # 船体加成数据（中文，everef ref-data）
├── api.php          # 缓存 API（POST 保存 / GET 加载）
├── esi.php          # ESI 代理（角色/军团/联盟查询）
├── fetch_attrs.py   # 从 ESI 拉取舰船属性脚本
├── fetch_traits.py  # 从 everef 拉取船体加成脚本
├── compare_ships.py # 对比本地与 ESI 最新数据脚本
├── .gitignore
└── README.md
```
