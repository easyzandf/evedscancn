...
- **会战报告包含**（参照 WarBeacon / evetools 的会战视图）：
  ...
  - **累计损失曲线** —— 一眼看出在哪一刻崩的
  - **时间轴** —— 中间一条**共用时间轴**，本方损失列在左、敌对列在右，纵向位置**严格等于发生时刻**（刻度按跨度取整步长，偏差为 0）
    - 纵向比例固定约 7px/分钟（夹在 240~2400px），所以不同会战之间的疏密可比
    - **密集时段行会重叠**，这是刻意的：不合并也不下推 —— 下推会让「位置＝时刻」变成谎话。实测最密的 5 分钟有 34 条损失，需要 612px 而线性只给得到 35px，必然叠成一堵墙
    - 想看清密集段，用上方的**星系筛选**缩小范围，或直接看**参战名单**
