...
- **会战报告包含**（参照 WarBeacon / evetools 的会战视图）：
  ...
  - **累计损失曲线** —— 一眼看出在哪一刻崩的
  - **时间轴** —— 中间一条**共用时间轴**，本方损失列在左、敌对列在右
    - 行按时间排序后逐条摆放，**保证至少一个行高间距** —— 绝不遮挡，每行完整可见，容器高度不够就拉伸
    - 两侧损失**合并排序后用同一套定位**，所以中间那条轴对左右两列都成立
    - **刻度从这套定位插值出来**，与实际布局一致 —— 不再画一条跟布局打架的线性刻度（那是之前的 bug：刻度是线性的、行却是固定间距堆叠的，两者对不上）
    - 为什么不做「位置严格＝时刻」：实测本方有 **5 条损失发生在同一时刻**，严格线性就得画在同一个 y 上，时间轴拉多高都分不开 —— 要分开需要 5760px/分钟、容器高 79 万像素。所以稀疏处位置即时刻（没触发避让），密集处拉伸；每行都带自己的时间戳，精确时刻不会丢
